...
function nearestPosition(x, y, scaleType, xScale, props){
  ...
  if (scaleType === 'time') {
    var valuesMap = props.results.d0Domain
      .filter(d => { return d != 'No Value' && d != 'Other'})
      .map(d => { return moment(d).toDate(); })

    xRange = valuesMap.map(function (v) { return xScale(v) })
  } else {
    xRange = xScale.range();
  }
...
export function updateTooltip(el, chartEl, chart){
  ...
  var xPos = el.getBoundingClientRect().left + xPos;

  var tTip = d3.select('.popover')[0][0];
  if (tTip) {
    var newY = d3.event.pageY - tTip.getClientRects()[0].height / 2;
    angular.element('.popover').css('left', xPos + 10)
    angular.element('.popover').css('top', newY)
  }
}
